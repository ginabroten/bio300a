...
# Omstrukturere data til langt format
lakselus_long <- lakselus_proposjoner |> 
  pivot_longer(cols = c(medikamentell_pr_anlegg, rensefisk_pr_anlegg, mekanisk_pr_anlegg), 
               names_to = "Tiltak", 
               values_to = "Verdi")

# Lag ggplot med facet_wrap etter ProduksjonsområdeId
ggplot(data = lakselus_long, aes(x = ProduksjonsområdeId, y = Verdi, fill = Tiltak)) +
  geom_col(position = "dodge") +  # Side om side barer
  facet_wrap(~ Tiltak, scales = "free_y") +  # Facet basert på tiltak
  labs(x = "ProduksjonsområdeId", y = "Verdi per anlegg", fill = "Tiltak") +
  theme_minimal()
